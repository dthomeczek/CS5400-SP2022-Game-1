...
class AI(BaseAI):
    ...
    def make_move(self) -> str:
        ...
        board = split_fen[0]
        next_player = split_fen[1]
        castle_var = split_fen[2]
        draw_turns = split_fen[4]
        total_turns = split_fen[5]
        board_list = board.split('/')

        # Creates a board
        new_board = parse_board(board_list)
        move_list = actions(self.player.color, new_board)

        # Gets a random move from a list of valid moves
        random_choice = random.randint(0, len(move_list) - 1)
        print("")
        print(move_list)
        print("")
        print(move_list[random_choice])
        #print("")
        #print_chess(new_board)
        random_move = move_list[random_choice]
        return random_move
